...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (id === adminUser.id) return NextResponse.json({ error: 'Impossible de vous bannir vous-même' }, { status: 400 });

  await prisma.user.update({ where: { id }, data: { status: 'BANNED' } });
  await logAudit({ userId: adminUser.id, action: 'USER_BAN', entity: 'User', entityId: id });
